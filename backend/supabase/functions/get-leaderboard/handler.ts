...
export async function handleGetLeaderboard(request: Request): Promise<Response> {
  ...
  try {
    ...
    const entriesRequest = serviceRequest(`challenge_leaderboard?select=rank,display_name,duration_ms,mistakes,move_count,submitted_at&challenge_id=eq.${challengeId}&order=rank.asc&limit=${limit}`);
    const ownRequest = installationId
      ? serviceRequest(`challenge_leaderboard?select=rank,display_name,duration_ms,mistakes,move_count,submitted_at&challenge_id=eq.${challengeId}&installation_id=eq.${installationId}&limit=1`)
      : Promise.resolve(null);
    const [response, ownResponse] = await Promise.all([entriesRequest, ownRequest]);
    ...
    const selfEntry = Array.isArray(ownRows) ? ownRows[0] ?? null : null;
