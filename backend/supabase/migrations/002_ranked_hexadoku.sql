...
alter table public.ranked_challenges
  ...
  add constraint ranked_challenges_puzzle_check check (puzzle ~ '^([0-9]{81}|[0-9A-G]{256})$');
...
alter table public.score_submissions
  add constraint score_submissions_final_board_check check (final_board ~ '^([1-9]{81}|[1-9A-G]{256})$');
